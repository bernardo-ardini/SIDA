...
ai4 = aic(m_bj4);    ai5 =    aic(m_bj5); ai4new   = aic(m_bj4new);
bic4 = bic(m_bj4);   bic5 =   bic(m_bj5); bic4new  = bic(m_bj4new);
sure4 = sure(m_bj4); sure5 = sure(m_bj5); sure4new = sure(m_bj4new);

%% CRITERIA WITH COMPLEXITY TERM
aic_vec = [ai1, ai2, ai3, ai4new, ai4, ai5];
bic_vec = [bic1, bic2, bic3, bic4new, bic4, bic5];
sure_vec = [sure1, sure2, sure3, sure4new, sure4,sure5];
...
xticklabels(["M1","M2", "M3", "M4new", "M4", "M5"])
title("AIC")
ax2 = subplot(1,3,2);
plot(bic_vec);
ax2.XTick = [1, 2, 3,4,5,6];
xticklabels(["M1","M2", "M3", "M4new","M4", "M5"])
title("BIC")
ax3 =subplot(1,3,3);
plot(sure_vec);
...
for h = hhs
    subplot(2,2,h)
    compare(data_v,m_bj1, m_bj2, m_bj3, m_bj4new, m_bj4, m_bj5,h,opt)
end
